...
export const createConfigFile = (options: Config) => {
  createFile("./saasmd.config.json", JSON.stringify(options, null, 2));
};
...
export const readConfigFile = (): (Config & { rootPath: string }) | null => {
  ...
  const configPath = path.join(process.cwd(), "saasmd.config.json");
...
export const sendEvent = async (
  ...
  const url = "https://saasmd-proxy-analytics.vercel.app";
  // const url = "http://localhost:3000";
  try {
    await fetch(url + `/api/send-event`, {
      method: "POST",
      headers: {
        "x-request-from": "saasmd",
      },
      body: JSON.stringify({
        event,
        config,
