...
export async function initProject(options?: InitOptions) {
  ...
  if (!nextjsProjectExists) {
    consola.fatal(
      "No Next.js project detected. Please create a Next.js project and then run `Saasmd init` within that directory."
    );
    process.exit(0);
  }
  ...
  if (!usingAppDirWithOutSrc && !usingAppDirWithSrc) {
    consola.fatal("Saasmd only works with the Next.js App Directory.");
    process.exit(0);
  }

  console.clear();
  console.log("\n");
  console.log(chalk(figlet.textSync("Saasmd", { font: "ANSI Shadow" })));
  const srcExists =
    usingAppDirWithSrc ??
  ...
  if (!tsConfigExists) {
    ...
    consola.fatal("Saasmd is only compatible with Typescript projects.");
    process.exit(0);
  }
  ...
  // consola.success("Saasmd initialized!");
